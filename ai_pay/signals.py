...
from ai_pay.models import POAssignment,POTaskDetails,POLangpair,PurchaseOrder
import logging
    ...
    try:
       with transaction.atomic():
        ...
        insert={'task_id':instance.task.id,'assignment':assign,'project_name':instance.task.job.project.project_name,
                ...
                'unit_type':instance.mtpe_count_unit}
        po_task=POTaskDetails.objects.create(**insert)
        POLangpair.objects.create(task_po=po_task,source_language=instance.task.job.source_language,target_language=instance.task.job.target_language)
        insert2={'client':instance.task.assign_to,'seller':instance.assigned_by,
