...
def renewal_list():
    ...
    subs =Subscription.objects.filter(billing_cycle_anchor__year=cycle_date.year,
                        billing_cycle_anchor__month=cycle_date.month,billing_cycle_anchor__day=cycle_date.day,status='active',plan__interval='year').filter(~Q(current_period_end__year=cycle_date.year ,
