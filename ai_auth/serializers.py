...
from ai_auth.models import (AiUser, AilaysaCampaigns, BillingAddress,UserAttribute,
                            ...
                            AilaysaCallCenter,AilaysaCallCenterFile,AilaysaEvent,DemoUserFormSubmission)
...
class AiUserDetailsSerializer(serializers.ModelSerializer):
    ...
    def get_signup_method(self,obj):
        soc = SocialAccount.objects.filter(user=obj)
        if soc.count()==0:
            return 0
        else:
            if soc.last().provider == 'google':
                return 1
            elif soc.last().provider == 'proz':
                return 2
...
class DemoUserFormSubmissionSerializer(serializers.ModelSerializer):
    class Meta:
        model = DemoUserFormSubmission
        exclude = ["id"]
