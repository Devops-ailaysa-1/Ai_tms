...
class GetVendorListViewNew(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset_all = AiUser.objects.select_related('ai_profile_info','vendor_info','professional_identity_info')\
                    ...
                    .distinct().exclude(id = user.id).exclude(is_internal_member=True).exclude(is_vendor=False).exclude(email='ams@ailaysa.com').exclude(is_active=False).exclude(deactivate=True)
