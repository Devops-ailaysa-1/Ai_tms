...
class IncompleteProjectListView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset_2 = Project.objects.select_related('voice_proj_detail').filter(voice_proj_detail__project_type_sub_category_id=2).filter(project_jobs_set__target_language=None).exclude(project_type_id__in = [6,7]).values('id')
        queryset = Project.objects.filter(Q(ai_user=user)\
                    ...
                    .exclude(id__in=queryset_2).exclude(project_type_id__in = [6,7]).order_by('-id').distinct()
        return queryset
