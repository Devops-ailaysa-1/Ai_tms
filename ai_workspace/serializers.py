		...
		try:
			with transaction.atomic():
				...
				if default_gloss_project:
					...
					default_gloss_project.project.project_name = f"{project.project_name}_glossary"
...
class GetAssignToSerializer(serializers.Serializer):
	...
	def get_internal_editors(self,obj):
		request = self.context['request']
		if obj.team:
			team = obj.team.internal_member_team_info.exclude(internal_member__is_active=False).filter(role=2)
...
def msg_send_vendor_accept(task_assign,input,reason):
    ...
    receivers =  receiver.team.get_project_manager_only if receiver.team else []
