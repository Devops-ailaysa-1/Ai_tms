...
class InitialTranslation(TranslationStage):
    ...
    def process(self, store_result, segments, style_prompt, gloss_terms, d_batches, document=None, batch_no=None, batch_instance=None):
        system_prompt = AdaptiveSystemPrompt.objects.get(stages=self.stage).prompt
        ...
        for para in segments:
            ...
            if store_result.stage_02:
                store_result.stage_02 += response_text
            else:
                store_result.stage_02 = response_text
...
class AdaptiveSegmentTranslator:
    ...
    def process_batch(self, segments, d_batches, batch_no):
        ...
        store_result = AllStageResult.objects.create(task_stage_result=store_result)
        segments,translated_segments = self.initial_translation.process(store_result, segments, style_guideline, self.gloss_terms, d_batches, self.document, batch_no, self.task_progress)
